...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
    ...
    static int LoginMode=1,ShowMode=1,Drawp,Drawq,InitDrawq=0;
    static int EditPage=1,CurrentPage=1,TotalPage=10;
    if (InitDrawq==0)
    {
        Drawq=ShowMode;
        InitDrawq=1;
    ...
    switch(message)
    {
        ...
        case WM_PAINT:
            ...
            if (initPaint==1)
            {
                ...
                MoveToEx(hdc,30,200+Drawp*40,NULL);
                LineTo(hdc,30,230+Drawp*40);
                LineTo(hdc,110,230+Drawp*40);
                LineTo(hdc,110,200+Drawp*40);
                LineTo(hdc,30,200+Drawp*40);
                //选择红笔
                SelectObject(hdc,hPenRed);
                MoveToEx(hdc,30,200+Drawq*40,NULL);
                LineTo(hdc,30,230+Drawq*40);
                LineTo(hdc,110,230+Drawq*40);
                LineTo(hdc,110,200+Drawq*40);
                LineTo(hdc,30,200+Drawq*40);
            }
            else
            {
                hdc=GetDC(hWnd);
