...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
    ...
    TCHAR szTextBuf[100],szHistory[5000];
    ...
    static int EditPage=1,PreviousPage=1,PosPage=1,TotalPage,PosMovie,TotalMovie;
        ...
        case WM_COMMAND:
            ...
            switch (wmId)
            {
                ...
                case 27:
                    total_history++;
                    GetLocalTime(&sys);
                    sprintf(history[total_history-1],TEXT("%s %4d/%02d/%02d %02d:%02d:%02d 星期%s\n"),movie[ShowMovie[(PosPage-1)*10+wParam-18]].Name,sys.wYear,sys.wMonth,sys.wDay,sys.wHour,sys.wMinute, sys.wSecond,weekname[sys.wDayOfWeek]);
                    break;
                ...
                case 100:
                    for (i=0;i<mins(TotalMovie-PosPage*10+10,10);i++)
                    {
                        ...
                        bg=(HBITMAP)LoadImage(
                            NULL,
                            movie[ShowMovie[(PosPage-1)*10+i]].Photo,
